...
public class CaveCell extends BoardCells implements CellsWithBonus{
    private Hero heroSlot;
    private Monster monsterSlot;
    private Boolean isHeroSlot = false;
    private Boolean isMonsterSlot = false;

    public CaveCell(String cellName, String cellValue, int[] cellLocation, String bgColour) {
    ...
    public void updateHeroPowers(Hero hero) {
        System.out.println("Cave has increased Hero's agility by 10%");
        hero.setAgility((int) (hero.getAgility() * 1.1));
    }

    public void resetHeroPowers(Hero hero) {
        System.out.println("Hero has left the cave, it's agility is reduced to the original value");
        hero.setAgility((int) (hero.getAgility() / 1.1));
    }    
}
