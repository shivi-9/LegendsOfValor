...
public class BushCell extends BoardCells implements CellsWithBonus{
    private Hero heroSlot;
    private Monster monsterSlot;
    private Boolean isHeroSlot = false;
    private Boolean isMonsterSlot = false;

    public BushCell(String cellName, String cellValue, int[] cellLocation, String bgColour) {
    ...
    public void updateHeroPowers(Hero hero) {
        System.out.println("Bushes have increased Hero's dexterity by 10%");
        hero.setDexterity((int) (hero.getDexterity() * 1.1));
    }

    public void resetHeroPowers(Hero hero) {
        System.out.println("Hero has left the Bushes, it's dexterity is reduced to the original value");
        hero.setDexterity((int) (hero.getDexterity() / 1.1));
    }
